...
parser.add_argument('--impute',type=float,default=2.0,help='Multiplier for imputation.')
parser.add_argument('--sample',action='store_true',help='Sample the resulting distribution')
...
if args.configspace == 'acotsp':
    ...
    cs.add_hyperparameter(hp.UniformIntegerHyperparameter('nnls', lower=5, upper=50*args.impute))                          ## imputed
    cs.add_hyperparameter(hp.UniformFloatHyperparameter('q0', lower=0.0, upper=1.0*args.impute))                           ## imputed
    #cs.add_hyperparameter(hp.CategoricalHyperparameter('dlb', choices=list(set(X['dlb'].tolist()))))                      ## imputed
    cs.add_hyperparameter(hp.UniformIntegerHyperparameter('dlb', lower=min(X['dlb']), upper=max(X['dlb'])))                ## imputed
    cs.add_hyperparameter(hp.UniformIntegerHyperparameter('rasrank', lower=0, upper=100*args.impute))                      ## imputed
    cs.add_hyperparameter(hp.UniformIntegerHyperparameter('elitistants', lower=1, upper=750*args.impute))                  ## imputed
...
if args.single:
    ...
    with open(base+'-importance'+args.replication+'.dat','w') as inf:
        ...
        for i in range(0,len(pnames)):
            ...
            print(base,args.replication,args.seed,len(X),args.ntrees,args.tag,pnames[i],imp['individual importance'],file=inf)
